...
def main(config):
    ...
    with torch.no_grad():
        for i, (data_batch) in enumerate(tqdm(valid_data_loader)):
    ...
    subject_vs_dice = sorted(subject_vs_dice, key=lambda x: x[0])
    ...
    ax.set_ylim([0,1])
    ax.yaxis.grid()
    plt.savefig(results_dir /'volume_vs_dice.png')
    ...
    ax.yaxis.grid()
    plt.tight_layout()
    plt.savefig(results_dir /'subject_vs_dice.png')
